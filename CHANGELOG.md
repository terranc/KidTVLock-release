...
### Fixed
- 第 4 步支持多个厂商桌面接续占用 HOME 的情况：每次设置失败后重新识别实际默认桌面，逐个处理可安全禁用的厂商桌面，验证成功即停止。
- 禁用前将多包恢复记录写入电视并回读确认；失败时尝试恢复本轮全部桌面，恢复未完成时保留记录，重启安装助手后仍可通过“恢复原桌面”继续恢复。
- 遇到初始化组件、系统设置、兜底桌面、用户选定的真实桌面或其它第三方应用时停止禁用；保留电视原有的禁用包状态。
- 默认桌面设置严格校验实际 HOME component，查询失败或无法识别时不再误报成功；设置与恢复命令统一使用用户 0。
